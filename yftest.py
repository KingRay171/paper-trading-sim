...
from PySide6.QtWidgets import (QWidget, QTabWidget, QGroupBox, QLabel, QTableWidget, QTableWidgetItem, QAbstractItemView, QHBoxLayout,
                               QSplashScreen, QPushButton, QDialog, QLineEdit, QComboBox, QRadioButton, QCalendarWidget, QCheckBox, QSizePolicy, QApplication, QProgressBar, QVBoxLayout, QTableView, QScrollArea, QButtonGroup, QSlider, QSpinBox, QDoubleSpinBox)
from PySide6.QtGui import QFont, QFontDatabase, QPixmap, QIcon, QPainter, QColor
from PySide6.QtCore import QRect, QCoreApplication, QStringListModel, QAbstractItemModel, QDateTime, Qt, SIGNAL, QPropertyAnimation
...
def update_portfolio_piechart():
    ...
    for idx, amount in enumerate(portfolio_amts):
        match portfolio_asset_types[idx].text:
            case "ETF":
                etf_amount += int(amount.text) * float(
                    portfolio_dialog.positions_view_groupbox.positions_view.item(idx - 1, 2).text()[1:])
            case "Liquidity":
                cash_amount += float(amount.text)
            case "Stock":
                stock_amount += int(amount.text) * float(
                    portfolio_dialog.positions_view_groupbox.positions_view.item(idx - 1, 2).text()[1:])
            case "Futures":
                futures_amount += int(amount.text) * float(
                    portfolio_dialog.positions_view_groupbox.positions_view.item(idx - 1, 2).text()[1:])
            case "Option":
                option_amount += int(amount.text) * float(
                    portfolio_dialog.positions_view_groupbox.positions_view.item(idx - 1, 2).text()[1:])
...
def update_wallet_table():
    ...
    for i in range(1, len(wallet_tickers)):
        ...
        total_return = (
            ticker_current - float(wallet_costbases[i - 1].text)) * float(wallet_amts[i].text)
        percent_change = round(
            total_return / (float(wallet_costbases[i - 1].text) * float(wallet_amts[i].text)) * 100, 2)

        # update the table with the new information

        # first cell in the row is the coin symbol
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 0, QTableWidgetItem(wallet_tickers[i].text.upper()))

        # second cell is the coin's performance icon
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 1, updateTickerIcon(ticker))

        # third cell is the coin's current price
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 2, QTableWidgetItem('${:0,.2f}'.format(ticker_current)))

        # fourth cell is the change in the coin's price from it's last close, in dollar and percent terms
        wallet_dialog.positions_view_groupbox.positions_view.setItem(i - 1, 3, QTableWidgetItem('${:0,.2f}'.format(
        ...
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 4, QTableWidgetItem('${:0,.2f}'.format(float(wallet_costbases[i - 1].text))))

        # sixth cell is the amount of the coin the user has (or is short)
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 5, QTableWidgetItem(wallet_amts[i].text))

        # seventh cell is the NLV the user has in the coin
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 6, QTableWidgetItem('${:0,.2f}'.format(ticker_current * float(wallet_amts[i].text))))

        # eighth cell is the user's net P/L on the position from when it was opened
        wallet_dialog.positions_view_groupbox.positions_view.setItem(i - 1, 7, QTableWidgetItem(
            '${:0,.2f}'.format(((ticker_current - float(wallet_costbases[i - 1].text)) * float(wallet_amts[i].text)))))

        # ninth cell is the total return on the position in % terms
        wallet_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 8, QTableWidgetItem('${:0,.2f}'.format(total_return) + " (" + str(percent_change) + "%)"))
...
def update_portfolio_table():
    ...
    for i in range(1, len(portfolio_tickers)):
        ...
        # calculate the return since the position was opened in dollar and percent terms
        total_return = (
            ticker_current - float(purchase_prices[i - 1].text)) * int(portfolio_amts[i].text)
        percent_change = round(
            total_return / (float(purchase_prices[i - 1].text) * float(portfolio_amts[i].text)) * 100, 2)
        # update the table with the new information
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 0, QTableWidgetItem(portfolio_tickers[i].text.upper()))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 1, updateTickerIcon(ticker))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 2, QTableWidgetItem('${:0,.2f}'.format(ticker_current)))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(i - 1, 3, QTableWidgetItem('${:0,.2f}'.format(
            ticker_current - ticker_last_close) + " (" + str(round(((ticker_current - ticker_last_close) / ticker_last_close * 100), 2)) + "%)"))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 4, QTableWidgetItem('${:0,.2f}'.format(float(purchase_prices[i - 1].text))))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 5, QTableWidgetItem(portfolio_amts[i].text))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 6, QTableWidgetItem('${:0,.2f}'.format(ticker_current * int(portfolio_amts[i].text))))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(i - 1, 7, QTableWidgetItem(
            '${:0,.2f}'.format(((ticker_current - float(purchase_prices[i - 1].text)) * int(portfolio_amts[i].text)))))
        portfolio_dialog.positions_view_groupbox.positions_view.setItem(
            i - 1, 7, QTableWidgetItem('${:0,.2f}'.format(total_return) + " (" + str(percent_change) + "%)"))
    ...
    if (tab_id == 1 and not tab2_isloaded):
        ...
        for er in ticker_yr_earnings_rev:
            ...
            yr_earnings_label_container.layout().addWidget(
                QLabel(f"{ticker_yr_earnings_rev['date']}: {earnings}"))
        ...
        for er in ticker_yr_earnings_rev:
            ...
            set.append(rev)
            if (rev > yr_revtrend_max):
                yr_revtrend_max = rev
            if (rev < yr_revtrend_min):
                yr_revtrend_min = rev
            yr_revtrend_label_container.layout().addWidget(
                QLabel(f"{ticker_yr_earnings_rev['date']}: {rev}"))
...
if __name__ == '__main__':
    ...
    all_tickers_list = pd.read_csv(
        CURRENT_DIR + "assets\stock_list.csv")['Symbol'].tolist()
    all_names_list = pd.read_csv(
        CURRENT_DIR + "assets\stock_list.csv")['Name'].tolist()
    ...
    for i in range(1, len(portfolio_amts)):
        price = yf.download(
            tickers=portfolio_tickers[i].text, period='5d').iloc[4][3]
    ...
    for i in range(1, len(wallet_amts)):
        try:
            price = yf.download(
                tickers=wallet_tickers[i].text, period='5d').iloc[4][3]
        except IndexError:
            price = yf.download(
                tickers=wallet_tickers[i].text, period='5d').iloc[3][3]
    ...
    portfolio_dialog.currentNAV.cash.setText(
        '${:0,.2f}'.format(portfolio_cash))
    ...
    tsi_panel_combobox.currentTextChanged.connect(lambda state: change_indicator_panel(
        "ta.momentum.tsi", state, selected_ta[get_indicator_index("ta.momentum.tsi")][2]) if tsi_checkbox.isChecked() else None)
    ...
    dpo_panel_combobox.currentTextChanged.connect(lambda state: change_indicator_panel(
        "ta.trend.dpo", state, selected_ta[get_indicator_index("ta.trend.dpo")][2]) if dpo_checkbox.isChecked() else None)
    ...
    kst_panel_combobox.currentTextChanged.connect(lambda state: change_indicator_panel(
        "ta.trend.KSTIndicator", state, selected_ta[get_indicator_index("ta.trend.KSTIndicator")][2]) if kst_checkbox.isChecked() else None)
    ...
    def kst_button_clicked():
        ...
        def ok_button_clicked():
            if (kst_checkbox.isChecked()):
                selected_ta[get_indicator_index("ta.trend.KSTIndicator")] = ("ta.trend.KSTIndicator", kst_panel_combobox.currentIndex(),
